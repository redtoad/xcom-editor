...
func Test_TRANSFER_DAT_RoundTrip(t *testing.T) {
	...
	assert.Equal(t, uint8(0), obj.Transfers[0].Origin)
	assert.Equal(t, uint8(1), obj.Transfers[0].Destination)
	assert.Equal(t, uint8(10), obj.Transfers[0].HoursLeft)
	assert.Equal(t, uint8(2), obj.Transfers[0].Type)
	assert.Equal(t, 5, obj.Transfers[0].ReferenceNumber)
	assert.Equal(t, uint8(3), obj.Transfers[0].Quantity)
