...
//go:generate stringer -type=Rank,Armor,Gender,Appearance -output=soldier_dat_string.go -linecomment
...
// SoldierFile contains all information abot the soldiers.
//
// https://www.ufopaedia.org/index.php/SOLDIER.DAT
type SoldierFile struct {
    Soldiers []SoldierData
}

func (s SoldierFile) SizeOf() int {
    return maxSoldiers * soldierByteLength
}

func (s SoldierFile) Pack(buf []byte, order binary.ByteOrder) ([]byte, error) {
...
func (s *SoldierFile) Unpack(buf []byte, order binary.ByteOrder) ([]byte, error) {
    s.Soldiers = make([]SoldierData, maxSoldiers)
    for i := 0; i < maxSoldiers; i++ {
        offset := i * soldierByteLength
...
type SoldierData struct {
    ...
    // 14-15 / 0E-0F (20+): SoldierData value a.k.a. victory point loss if dies on a
    ...
    // can be entered when editing the field within the game's SoldierData display, but
...
func (s *SoldierData) GoString() string {
    return s.String()
}

func (s *SoldierData) String() string {
    if s.Rank == DeadOrUnused {
        return "{SoldierData <dead or unused>}"
    } else {
        return fmt.Sprintf("{SoldierData %v %s}", s.Rank, s.Name)
    }
}
...
const (
    ...
    Colonel
    Commander
    DeadOrUnused Rank = -1 // Dead or Unused
)
...
const (
    NoArmor        Armor = iota
    PersonalArmour       // Personal Armour
    PowerSuit            // Power Suit
    FlyingSuit           // Flying Suit

    // Can be hacked to

    SectoidArmor    // Sectoid Armour
    SnakemanArmor   // Snakeman Armour
    EtherealArmor   // Ethereal Armour
    MutonArmor      // Muton Armour
    FloaterArmor    // Floater Armour
    CelatidArmor    // Celatid Armour
    SilacoidArmor   // Silacoid Armour
    ChryssalidArmor // Chryssalid Armour
)
